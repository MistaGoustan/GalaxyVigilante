...
public class KeepLives : MonoBehaviour {
	...
	void UpdateLife () {
		// set init position
		Life.transform.position = new Vector2 (-12.5f, 6.7f);
		...
		for(int i = 0; i < LifeVal; i++){
			...
			Instantiate (Life,
			             Life.transform.position = new Vector2 (Life.transform.position.x + pos, Life.transform.position.y),
			             Life.transform.rotation);
		}
