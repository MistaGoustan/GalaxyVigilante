...
public class ShipFire : MonoBehaviour {
	...
	// this is the Shot prefab / rate of fire
	...
	void Fire(){
		...
		Instantiate(Shot_Object, 
		            new Vector2 (Ships_Transform.position.x + 0.9f, Ships_Transform.position.y), 
		            Quaternion.identity);
		Sound ();
	...
	void Update () {
		...
		if (Input.GetButtonDown ("Fire") && ReloadTime >= 0.15f && ToggleShot == false ||
		    Input.GetAxis ("Fire") == -1 && ReloadTime >= 0.15f && ToggleShot == false && TriggerPressed == false) {
			TriggerPressed = true;
			Fire();
			// reset
			ReloadTime = 0.0f;
		...
		if (Input.GetButton ("Fire") && ReloadTime >= 0.40f && ToggleShot == false ||
		    Input.GetAxis ("Fire") == -1 && ReloadTime >= 0.40f && ToggleShot == false){
			TriggerPressed = true;
			Fire();
			// reset
			ReloadTime = 0.0f;
